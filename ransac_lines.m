function [lines, inlier_sets] = ransac_lines(points, max_lines, thresh, min_inliers)
    % Simple RANSAC to extract multiple lines iteratively
    remaining = points;
    lines = {}; inlier_sets = {};
    iters = 1200;
    while size(remaining,1) >= min_inliers && length(lines) < max_lines
        ...
        if best_count >= min_inliers
            lines{end+1} = best_line; %#ok<AGROW>
    ...
    % Optionally refine lines by re-fitting to inliers using least squares
    for i=1:length(lines)
        inpts = points(inlier_sets{i},:);
        if size(inpts,1) >= 2
            % fit line y = mx + q by PCA for stability
            mu = mean(inpts,1);
            C = (inpts - mu)' * (inpts - mu);
            [V,D] = eig(C);
            [~, idx] = max(diag(D));
            dir = V(:,idx)';
            % compute normalized ax + by + c
            a = -dir(2); b = dir(1);
            norm_ab = sqrt(a^2 + b^2);
            a = a / norm_ab; b = b / norm_ab;
            c = -(a*mu(1) + b*mu(2));
